...
const [limitWarning, setLimitWarning] = useState('');
  ...
  useEffect(() => {
    ...
    const fetchReceiverDetails = async () => {
      try {
        const response = await axios.get(`/api/patient-profile-information/${id}`)
        setReceiverDetails(response.data.patient)
      } catch (err) {
        console.log("err when fetch receiverDetails", err)
      }
    }
    if (id) {
  ...
  const fetchHistory = async (senderId, receiverId) => {
    try {
      const response = await axios.post(`/api/consultant-chat-history/${senderId}`);
  
      const chats = response.data.chats || [];  

      const filterPreviousMessages = chats.map((chat) => {
        const { patientId, patientDetails, messages } = chat.chatDetails;
          const lastMessage = messages.reduce((latest, current) => {
          return new Date(current.timestamp) > new Date(latest.timestamp) ? current : latest;
          }, messages[0]);
        return {
          firstName: patientDetails?.information?.firstName || 'Unknown',
          patientId,
      ...
      const matchingChats = chats.filter(
        (chat) => Number(chat.chatDetails.patientId) === Number(receiverId)
      );
...
newSocket.on('messageLimitReached', (data) => {
  ...
  const handleBackButton = () => {
    history.push("/doctor/doctor-dashboard"); // Corrected push method
  }; 
                  ...
                  <div className="slimscroll-active-sidebar">
                    ...
                    <div className="sidebar-body chat-body" id="chatsidebar">
                      ...
                      <ul className="user-list">
                        {previousMessages && previousMessages.map((item, index) => (

                        <li className="user-list-item" key={index}>
                          <Link to={`/chat-doctor/${item.patientId}`}>
                            ...
                            <div className="users-list-body">
                              <div>
                                ...
                                <p>{item?.lastMessage?.messageType == "text" && item?.lastMessage?.message}</p>
                              </div>
                ...
                <div className="container d-flex justify-content-center default-chat-message-window">
                  <div className="card d-flex align-items-center" style={{ textAlign: 'center' }}>
                    ...
                    <div className="card-body">
                      <h5 className="card-title">Welcome to your Patient Chat!</h5>
                      <p className="card-text">Select a patient to begin your consultation and get the response you need.</p>
                    </div>
                  </div>
                </div>
                :
                    ...
                    <div className="chat-header">
                      <div className="user-details">
                        ...
                        <figure className="avatar ms-1 avatar-online">
                          <img src={receiverDetails && receiverDetails.profileImage
                            ? receiverDetails.profileImage : "/assets/images/doctor-thumb-01.png"
                          } alt="image" />
                        </figure>
                        <div className="mt-1">
                          <h5>{receiverDetails?.information?.firstName} {receiverDetails?.information?.lastName}</h5>
...
<label htmlFor="media-file-input"><span className="p-2">
                    ...
                    <div className="smile-foot emoj-action-foot">
                      ...
                      <div className="emoj-group-list-foot down-emoji-circle">
                        <ul>
                          <li>
                            <Link to="#">
                              <img src={emoj1} alt="Icon" />
                            </Link>
                          </li>
                          <li>
                            <Link to="#">
                              <img src={emoj2} alt="Icon" />
                            </Link>
                          </li>
                          <li>
                            <Link to="#">
                              <img src={emoj3} alt="Icon" />
                            </Link>
                          </li>
                          <li>
                            <Link to="#">
                              <img src={emoj4} alt="Icon" />
                            </Link>
                          </li>
                          <li>
                            <Link to="#">
                              <img src={emoj5} alt="Icon" />
                            </Link>
                          </li>
  ...
  onKeyDown={(e) => {
    if (e.key === 'Enter' && !e.shiftKey && !limitReached) { // ADD !limitReached check
      e.preventDefault();
      handleSendMessage(e);
    }
  }}
  ...
  style={{
    maxHeight: '100px',
    overflowY: 'auto',
    opacity: limitReached ? 0.5 : 1 // ADD THIS
  }}
/>
